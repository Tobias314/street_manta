...
Future<http.Response> uploadGeoCapture(GeoCapture geoCapture,
  ...
  for(var path in paths) {
    zipPaths.add((await File(path).rename(inputDir.path + '/' + basename(path))).path);
  }
  File file = File(inputDir.path + '/geo_capture.pb');
  file.writeAsBytes(geoCapture.writeToBuffer());
  zipPaths.add(file.path);

  var zip_file = File(tempDir.path + '/geo_capture.zip');
  await ZipFile.createFromFiles(sourceDir: inputDir, files: zipPaths.map((path)=>File(path)).toList(), zipFile: zip_file);
  ...
  do {
    ...
    var request = http.MultipartRequest(
        "POST", Uri.parse('${globals.backendUrl}/api/geo_capture/upload/${Uuid().v4()}'));
