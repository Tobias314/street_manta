...
Future<List<GeoPhoto>> fetchGeoPhotosForRegion(LatLngBounds bounds) async {
  final globals = await Globals.getInstance();
  String route =
      '/api/geophoto/get_for_region/${bounds.south},${bounds.west},${bounds.north},${bounds.east}';
  Map<String, String> headers = {
    'Content-type': 'application/json',
    'Accept': 'application/json',
    'Authorization': 'Bearer ${await getUserToken()}',
  ...
  if (geoPhotoResponse.statusCode == 200) {
    ...
    return geoPhotosJson.map((geoPhotoJson) {
      return GeoPhoto.fromJson(geoPhotoJson);
    }).toList();
  } else {
...
Future<Image> fetchImage(String imageId, {bool fetchThumbnail = false}) async {
  final globals = await Globals.getInstance();
  Map<String, String> headers = {
    'Content-type': 'application/json',
    'Accept': 'image/png',
  ...
  Uri uri = Uri.parse(
      '${globals.backendUrl}/api/geophoto/${fetchThumbnail ? "fetch_thumbnail" : "fetch_image"}/$imageId');
